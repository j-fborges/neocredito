...
export default function ESignDossierPanel() {
  ...
  if (loading) return <div className="p-6">{translations.dossier.loading}</div>;
  if (error)
    return (
      <div className="p-6 text-red-500">
        {translations.dossier.error} {error}
      </div>
    );
  if (!dossier)
    return <div className="p-6">{translations.dossier.notFound}</div>;
